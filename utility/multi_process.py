...
def do_task(data1: int, data2: int, data3: int) -> int:
    data = data1 + data2 + data3
    logger.info("Task data : [%d]", data)
    return data


def worker(func, q: JoinableQueue, stop_event: Event):
...
def run(
    ...
    for i in range(n_workers):
        p = Process(name=f"Worker-{i:02d}", daemon=True, target=worker, args=(func, q, stop_event))
        procs.append(p)
        p.start()
    try:
        # Put tasks on queue
